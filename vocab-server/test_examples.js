...
async function main() {
  ...
  for (const word of words) {
    ...
    // Check for actual metadata in examples
    const allExamples = payload.senses?.flatMap(s => s.examples?.map(e => e.en.toLowerCase()) || []);
    if (allExamples) {
      const badPatterns = ['uk', 'us', 'browser', 'vocabulary', 'wikipedia', 'share', 'to top'];
      const badExamples = allExamples.filter(e => 
        badPatterns.some(p => e === p || e.startsWith(p + '\n'))
      );
      if (badExamples.length > 0) {
        console.log('\n⚠ PROBLEMATIC EXAMPLES:', badExamples.slice(0, 3));
      } else {
        console.log('\n✓ All examples clean (no metadata)');
      }
    }
  }
}
