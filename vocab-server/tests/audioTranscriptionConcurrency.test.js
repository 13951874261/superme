...
async function testAudioTranscriptionService() {
  ...
  // 2. 验证业务语言日志（已取消云端 STT 降级）
  assert.ok(content.includes('[语音识别] 正在通过专属语音引擎解析学员发音音频:'), '必须包含专属语音引擎识别日志');
  assert.ok(content.includes('[语音调度] 专属语音引擎当前正忙，拒绝新请求:'), '必须包含并发正忙拒绝业务日志');
  assert.ok(content.includes('[语音容灾] 专属语音引擎'), '必须包含专属语音引擎失败日志');
  assert.ok(content.includes('不再降级'), '必须明确不再降级云端 STT');
  assert.ok(content.includes('[发音规整] 正在对识别文本进行口语表达优化与标点还原:'), '必须包含发音表达规整业务日志');
  assert.ok(!content.includes('audio-to-text'), '不得再调用 Dify audio-to-text 降级');
