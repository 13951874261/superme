/**
 * AE-JUD-01：高阶审美研判结果防串台（拒收英语词典式输出）
 */

function looksLikeVocabCrossover(feedback) {
...
function hasRequiredAestheticsShape(raw) {
  const score = Number(raw?.score);
  const feedback = typeof raw?.feedback === 'string' ? raw.feedback.trim() : '';
  return Boolean(feedback) && Number.isFinite(score) && score >= 0 && score <= 10 && typeof raw?.is_passed === 'boolean';
}

function buildSocialFallbackFeedback(sceneCategory) {
...
function ensureAestheticsResult(raw, sceneCategory) {
...
module.exports = {
  ...
  hasRequiredAestheticsShape,
  buildSocialFallbackFeedback,
