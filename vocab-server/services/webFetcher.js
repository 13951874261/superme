...
async function fetchUrlContent(urlString) {
  ...
  if (!isValid) {
    throw new Error('网页抓取失败：目�?URL 格式非法或属于受限的网络地址');
  }

  const apiKey = process.env.DIFY_FETCH_API_KEY || 'sk-899c9c34738f61b5-2u53op-6ed8a313'; // 默认回落到公�?Key
  const endpointBase = process.env.FETCH_ENDPOINT_BASE || 'https://23.95.214.232/v1/';
  ...
  // 3. 数据解析与清�?  let markdown = data.markdown || data.content || '';
  if (!markdown) {
    throw new Error('网页解析成功，但未能在目标网页中提取出有效文本内�?);
  }
  ...
  if (!title) {
    ...
    title = titleMatch ? titleMatch[1].trim() : '未命名网页材�?;
  }
