...
# 5. 在新按钮后添加压力因素选项的渲染
# 我们在 </div className="flex flex-col gap-4 mb-6 ..."> 结束前加上压力因素的选择 UI
# 在原文件里，</div className="flex flex-col gap-4 mb-6 ..."> 的结尾之前是：</div> (第 505 行后面)
# 让我们用 python 寻找特定位置并插入压力选项：
search_pos = content.find('</div>\n              {isCacheableDuration && (pregenStatus === \'missing\'')
if search_pos >= 0:
    pressure_ui = """
              {/* 压力因素选择器 */}
              <div className="flex flex-wrap items-center gap-3 mt-3 relative z-10 border-t border-white/5 pt-3">
...
"""
    ...
    print("Added pressure UI successfully!")
else:
    print("Could not find insertion position for pressure UI")
