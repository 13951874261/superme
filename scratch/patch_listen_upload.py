...
# 在 /api/listen/pregenerated/cron-run 后面插入新接口
new_interface = """
// ==========================================
// 听力材料上传接口
// ==========================================
...
# 找到插入位置
insert_marker = "app.post('/api/listen/pregenerated/cron-run', async (req, res) => {"
insert_pos = content.find(insert_marker)
if insert_pos >= 0:
    # 找到这个接口的结束位置（下一个 // ====== 标记）
    next_marker = content.find("\n// ==========================================\n", insert_pos)
    if next_marker > 0:
        content = content[:next_marker] + new_interface + content[next_marker:]
        print("Inserted upload audio interface")
    else:
