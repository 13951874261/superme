...
export default function DashboardTab() {
  ...
  // 3. 自动拉取后端 SQLite 数据库中持久化的今日长文（支持多条件组合查询与自动打底生成）
  ...
  useEffect(() => {
    ...
    const fetchPersistedArticle = async () => {
      try {
        ...
        const res = await getDailyExtractedArticle(getAppUserId(), genre, cefrLevel);
        if (!active) return;
        ...
        } else if (!res.found && intelSource === '每日系统生成') {
          const currentKey = `${genre}_${cefrLevel}_${theme}`;
          if (lastAutoGeneratedKeyRef.current !== currentKey && !isAutoGenerating) {
            lastAutoGeneratedKeyRef.current = currentKey;
            handleAutoGenerate();
          }
        }
      } catch (e) {
