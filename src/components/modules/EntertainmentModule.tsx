...
export default function EntertainmentModule() {
  ...
  const [dailyPushLoading, setDailyPushLoading] = useState(false);

  const [dailyPushSource, setDailyPushSource] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled && res.ok && payload.success && payload.result) {
          ...
          setDailyPush({
            ...
            category: r.category === 'social' ? 'social' : 'aesthetics',
        ...
        if (!cancelled) console.warn('[Entertainment] daily push unavailable:', e);

      } finally {

        if (!cancelled) setDailyPushLoading(false);
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const res = await fetch('/api/aesthetics/daily-push/regenerate', {
                          ...
                          console.warn('[Entertainment] regenerate failed:', e);

                        } finally {

                          setDailyPushLoading(false);

                        }

                      }}

                      disabled={dailyPushLoading}

                      className="flex items-center gap-1 rounded-lg bg-zinc-900 px-3 py-1.5 text-[11px] text-white disabled:opacity-50"
