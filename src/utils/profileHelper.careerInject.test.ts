...
test('getInjectedUserCurrentProfile 含职业路径与短板 L3 Graph', () => {
  ...
  Object.defineProperty(globalThis, 'localStorage', { value: fakeLocalStorage, configurable: true });
  try {
    store.set('User_Current_Profile', '英国听辨断层');
    store.set('user_current_profile', '英国听辨断层');
    ...
    store.set('user_memory_layers', JSON.stringify({
    ...
    assert.equal(out.includes('能力匹配度=41%'), true);
    assert.equal(out.includes('英国听辨断层'), true);
    assert.equal(out.includes('Accent:UK'), true);
    assert.equal(out.includes('Graph:'), true);
  } finally {
    Object.defineProperty(globalThis, 'localStorage', { value: prev, configurable: true });
  }
});
